...
export class MoviesController {
  ...
  @ApiOperation({ summary: 'Fetches playing movies or precise movie based on title' })
  @ApiSecurity('jwt', ['jsonwebtoken'])
  @ApiResponse({ status: 201, description: 'List of movies' })
  @ApiResponse({ status: 403, description: 'Unauthorized' })
  @ApiQuery({ name: 'page', description: 'Page to fetch' })
  @ApiQuery({ name: 'search', description: 'Name of movie' })
  @Get('')
  @UseGuards(AuthGuard)
  async getPlaying(
